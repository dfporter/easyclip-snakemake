...
class scheme():
    ...
    def __str__(self):
        li = "scripts.scheme.scheme object with the following data frame (self.scheme_df):"
    ...
    def generate_mappings(self):
        ...
        for exp, p6, p3, gene, replicate in zip(
            df.Experiment, df.L5_BC, df.L3_BC, df.Gene, df.Replicate):
        ...
        for row in df.to_dict('records'):
            (exp, p6, p3) = (row['Experiment'], row['L5_BC'], row['L3_BC'])
        ...
        for row in df.to_dict('records'):
            self.p6_to_gene[row['L5_BC']] |= set([row['Gene']])
            self.p6_to_list_of_dicts[row['L5_BC']].append(row)
            
    def read_scheme(self, scheme_fname=''):
        if 'xls' in scheme_fname.split('.')[-1]:
        ...
        if 'P3_BC' in self.scheme_df.columns and 'L3_BC' not in self.scheme_df.columns:
            self.scheme_df['L3_BC'] = self.scheme_df['P3_BC']
            
        # We often add a two letter prefix to the P6 barcode, using non-DNA letters.
        # We chop that off here.
        self.scheme_df['L5_DNA'] = [x.upper() for x in self.scheme_df['L5_BC']]
        self.scheme_df['L5_DNA'] = [''.join([letter for letter in barcode if (
            letter in ['A', 'T', 'C', 'G'])]) for barcode in self.scheme_df['L5_DNA']]
...
#            print(self.xl_by_prot_from_file)
    ...
    def set_long_form_filenames(self):
        ...
        '{exp}_{gene}_{p6}_{p3}.fastq'.format(
            ...
            scheme_df['Experiment'], scheme_df['Gene'], scheme_df['L5_BC'], scheme_df['L3_BC'])
        ]
        ...
        for (p6, p6_dna, p3, long_fname_R1, long_fname_R2) in zip(
            scheme_df['L5_BC'], scheme_df['L5_DNA'], scheme_df['L3_BC'],
