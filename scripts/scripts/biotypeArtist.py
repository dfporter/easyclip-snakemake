...
import scripts.intronExonSplit
import scripts.stacked_bargraph
import scripts.biotypeDf
importlib.reload(scripts.biotypeUtils)
importlib.reload(scripts.stacked_bargraph)
...
def dataframe_of_target_biotypes(
    ...
    bio = scripts.biotypeDf.biotypeDf(df=df[numeric_columns + ['Gene type']].copy())
    ...
    for protein in set([from_fname(col) for col in numeric_columns]):
        ...
        bio = scripts.biotypeDf.biotypeDf(df=df[cols + ['Gene type']].copy())
        ...
        bio = scripts.biotypeDf.biotypeDf(df=target_df[cols + ['Gene type']].copy())
