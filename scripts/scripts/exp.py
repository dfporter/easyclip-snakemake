...
import scripts.remove_short_reads
import scripts.collapse_duplicates
import scripts.statsForCounts
import scripts.split_r1
import scripts.split_r2
...
importlib.reload(scripts.countsO)
...
importlib.reload(scripts.remove_short_reads)
importlib.reload(scripts.collapse_duplicates)
importlib.reload(scripts.statsForCounts)
importlib.reload(scripts.split_r1)
importlib.reload(scripts.split_r2)
...
class exp(scripts.mapping.mappingMethods):
    ...
    def autogenerate_paths(top_dir: str) -> dict:
        file_paths = {
            'scheme': top_dir + '/samples.txt',
            'samples': top_dir + '/samples.txt',
            'beds': top_dir + '/beds/',
    ...
    def set_default_paths_if_needed(self):
        ...
        self.file_paths.setdefault('cutadapt', self.file_paths['fastq'] + '/ready_to_map/')
...
#            bed_folder=self.file_paths['beds'],
    ...
    def bed_to_wig(
        ...
        sbedgraphs = scripts.bedgraphs.set_of_bedgraphs(
    ...
    def annotate_counts_file(self):
        ...
        self.counts = scripts.countsO.countsO(
...
#########################################################
    ...
    def split_by_barcode(self):
        ...
        scripts.split_r1.split_a_fastq_by_R1(
            self.file_paths['R1_fastq'],
        ...
        scripts.split_r2.split_a_dir_by_R2(
    ...
    def preprocess_split_barcodes_folder(self, dirname: Union[str, None] = None):
        ...
        for r2_fastq in glob.glob(dirname + '/*_R2_*.fastq'):
            ...
            self.log['clip_by_r2'] = scripts.clip_adapters.clip_by_r2(
                r1_fastq, r2_fastq, r2_clipped_dir)

        for r2_fastq in glob.glob(r2_clipped_dir + '*_R2_*.fastq'):
            (p6, p3, r1_fastq) = get_p6p3_r1_fastq(r2_fastq)
            
            self.log['clip_by_r1'] = scripts.clip_adapters.clip_by_r1(
        ...
        (r1_fnames, r2_fnames) = scripts.combine_fastqs_for_mapping.make_mappable(
        ...
        for r1_fname, r2_fname in zip(r1_fnames, r2_fnames):
            cut_fnames.append(scripts.clip_adapters.clip_a_file_pair_using_cutadapt(
        ...
        scripts.combine_fastqs_for_mapping.concatenate_fastqs(
