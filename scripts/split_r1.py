...
def split_a_fastq_by_R1(read1, read2=None, output_dir='./', scheme_fname=''):
    ...
    if 'P6_BC' in scheme.columns and 'L5_BC' not in scheme.columns:
        scheme['L5_BC'] = scheme['P6_BC']

    # Open output folders.
    for barcode, name in zip(scheme.L5_BC, scheme.Name):
...
def read_scheme(filename, rev_comp=False):
    """label\tbarcode\n"""
    print(f"scheme filename: {filename}")
