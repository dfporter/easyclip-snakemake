...
class metaExp():
    ...
    def add_exp(self, exp_obj: scripts.exp.exp):
    ...
    def combine_scheme_files(self):
        ...
        for name, exp in self.exps.items():
            ...
            if not hasattr(exp, 'scheme'):
                schemes[name] = scripts.scheme.scheme(exp.file_paths['scheme'])
            else:
    ...
    def combine_scheme_stat_files(self):
        ...
        for name, exp in self.exps.items():
            ...
            _d = scripts.scheme.scheme(scheme_with_stat).scheme_df.to_dict('records')
    ...
    def combine_counts_files(self, verbose=False):
        ...
        for name, exp in self.exps.items():
            ...
            _counts = scripts.countsO.countsO(
...
#            ann_counts_fname, sep='\t')
    ...
    def read_scheme(self, fname: Union[None, str] = None) -> scripts.scheme.scheme:
        
        if fname is not None:
            self.file_paths['scheme'] = fname
        self.scheme = scripts.scheme.scheme(self.file_paths['scheme'])
